...
function vote(obj, id, type) {
    ...
    var vtype = obj.find("i").html();
    vtype = $.trim(vtype);
    console.log(vtype);
    if (vtype == 'liked' && type == 'up') {
        k = 0;
    } else if (vtype == 'disliked' && type == 'up') {
        k = 1;
    } else if (vtype == '' && type == 'up') {
        k = 1;
    } else if (vtype == 'disliked' && type == 'down') {
        k = 0;
    } else if (vtype == 'liked' && type == 'down') {
        k = -1;
    } else if (vtype == '' && type == 'down') {
        k = -1;
    }
    var url_str = "/vote?id=" + id + "&k=" + k;
    $.ajax({
        url: url_str,
        cache: false,
        dataType: 'json',
        ...
        success: function (data) {
            if (type == 'up') {
                obj.find("b").html(data.up);
                obj.removeClass('digg-a').addClass('vote-actived');

                obj.next('a').removeClass('vote-actived').addClass('digg-a');
                obj.next('a').find("i").html(data.vtype);
                obj.next('a').find("b").html(data.down);
            } else if (type == 'down') {
                obj.find("b").html(data.down);
                obj.removeClass('digg-a').addClass('vote-actived');
                
                obj.prev('a').removeClass('vote-actived').addClass('digg-a');
                obj.prev('a').find("i").html(data.vtype);
                obj.prev('a').find("b").html(data.up);
            }
            obj.find("i").html(data.vtype);
        }
    });
}

//function comment(obj,id,tid,cid,pid){
function comment(obj, id, type) {
    $(obj).attr("disabled", true).siblings('.status').show();
    if (type == 'parent' || type == 'ajaxparent')
        var textid = '#tt_' + id;
    else
        var textid = '#t_' + id;
    var text = $(textid).val();
    var url_str = "/ajax/comment";
    $.ajax({
        url: url_str,
        cache: false,
        type: 'POST',
        ...
        data: {id: id, type: type, text: text, FORMHASH: $('.FORMHASH').val()},
        success: function (feed_back) {
            //feed_back = feed_back.replace(/(^\s*)|(\s*$)/g, "");
            if (typeof feed_back.msg !== 'undefined') {
                //MessageBox('warning', feed_back.msg);
                show_message(feed_back.msg, 3, 'alertBox');
            } else {
                if (type == 'parent') $('.nestedlisting').prepend(feed_back);
                else if (type == 'ajaxparent') $(obj).closest('.usertext-edit').before(feed_back);
                else $('#child_' + id).prepend(feed_back);
                $(textid).val('');
                cancle_usertext('edit_' + id);
                show_message('发布评论 评论积分 +1', 3, 'rightBox');
            }
            $(obj).attr("disabled", false).siblings('.status').hide();
        }
    });
}


function cancle_usertext(edit) {
    var obj = '#' + edit;
    $(obj).remove();
}
...
function reply(obj, id) {
    ...
    cancle_usertext('edit_' + id);
    e = $(".usertext.usertext-edit:first").clone(!0);
    e.attr('id', 'edit_' + id);
    e.css('margin-top', '5px');
    e.css('display', 'block');
    e.find('textarea').attr('id', 't_' + id);
    var button = '<button type="submit" onclick="comment(this,' + id + ',\'child\')" class="btn btn-primary btn-xs">提交</button>' +
        '<button class="btn btn-default btn-xs" onclick="cancle_usertext(\'edit_' + id + '\')">取消</button>' +
        '<span class="status" style="display:none">提交中...</span>';
    e.find('.usertext-buttons').html(button);
    e.insertAfter($(obj).parent());
    e.find('textarea').focus();
}

function change_state(obj, id, type, act) {
    if (!login) {
        window.location = "/login";
    }
    var url_str = "/ajax.php?code=report&id=" + id + "&type=" + type + "&act=" + act;
    var method = "POST";
    if (type === "topic" && act === "del") {
        url_str = "/t/" + id;
        method = "DELETE"
    }
    $(obj).html('提交中...');
    $.ajax({
        ...
        success: function (feed_back) {
            if (type == 'topic')
                $(obj).closest('.link').hide('fast');
            else if (type == 'reply')
                $(obj).closest('.thing').hide('fast');
        }
    });
}

function save(obj, id, act, via) {
    if (!login) {
        window.location = "/login";
    }
    via = typeof via !== 'undefined' ? via : 'lvv2';
    if (act == 'add') {
        $(obj).find('span').removeClass('icon-collect').addClass('collect-actived');
    }
    var url_str = "/ajax/save?act=" + act + "&id=" + id + "&via=" + via;
    ...
    $.ajax({
        ...
        success: function (feed_back) {
            ...
            if (feed_back == 'success') {
                if (act == 'add')
                    $(obj).replaceWith('<a href="javascript:;" class="collect-a" onclick="save(this,' + id + ',\'remove\',\'' + via + '\')"><span class="hand-icon collect-actived"></span></a>');
                else
                    $(obj).replaceWith('<a href="javascript:;" class="collect-a" onclick="save(this,' + id + ',\'add\',\'' + via + '\')"><span class="hand-icon icon-collect"></span></a>');

            } else {
                show_message(feed_back, 3, 'alertBox');
            }
        }
    });
}

function clickCounter(tid) {
    var myAjax = $.post("/ajax/clickCounter",
        {
            tid: tid
        },
        function (d) {
...
//编辑标签窗口打开与关闭 20180325
//action= updatetopic addimgtopic tagsupdate
function floatDivToggle(tid, action) {
    var data = {tid: tid};
    showDialog('showDialog_' + tid, 'ajax', '编辑', {url: '/tool' + action, post: data}, 400);
    /*
    var eid='Div_float_area';
    var tid='undefined'==typeof(tid)?'':tid;
    var eidVal=$("#"+eid).html();
    if(''==eidVal){
        var myAjax=$.post("/ajax.php?code="+action,
            {
                tid:tid,
...
//标签编辑 20180325
function tagsDoUpdate(tid, tag) {
    if (tag != '') {
        tags = tag;
    } else {
        var tags = document.getElementById("ajax_tag_update").value;
        if ('' == tags) {
            show_message('请输入内容', 2, 'alertBox');
            return false;
        }
    }
    closeDialog("showDialog_" + tid);
    var myAjax = $.post("/tool/tag/tagUpdate",
        {
            tags: tags,
            tid: tid
        },
        function (d) {
            d = d.replace(/(^\s*)|(\s*$)/g, "");
            if ('' != d && d != 'success') {
                show_message(d, 2, 'alertBox');
                return false;
            } else {
                show_message('成功', 2, 'rightBox');
            }
        }
    );
}
//标签编辑 20231202
function share2tg(tid) {
    var myAjax = $.post("/tool/share2tg",
        {
            tid: tid
        },
        function (d) {
...
//标签编辑 20180325
function add2instantView(url, tid) {
    var myAjax = $.post("/tool/add2instant-view",
        {
            url: url,
            tid: tid
        },
...
$(function () {
    ...
    $('#noLoginImg').click(function () {
        if ($('#menuBox').is(":visible") && !$('#nologinOprBox').is(":visible")) {
            $("#menuBox").toggle();
        }
        $("#nologinOprBox").toggle();
    });
    /*
    $('#shownav').click(function() {
        if($('#menuBox').is(":visible")&&!$('nav').is(":visible")){
            $("#menuBox").toggle();
        }
        $("nav").toggle();
    });
...
var shareTo = {
    shareToTwitter: function (title, url) {
        if (title.length > 208) {
            title = title.substr(0, 205) + "..."
        }
        //var param={text:title,url:url,via:"lvv2com",related:"lvv2com"};
        var param = {text: title, url: url};
        var temp = [];
        for (var p in param) {
            temp.push(p + "=" + encodeURIComponent(param[p] || ""))
        }
        var _u = "https://twitter.com/intent/tweet?" + temp.join("&");
        window.open(_u, "", "width=700, height=480, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, location=yes, resizable=no, status=no")
    },
    shareToFacebook: function (title, url) {
        url = encodeURIComponent(url);
        var _u = "https://www.facebook.com/sharer/sharer.php?u=" + url;
        window.open(_u, "", "width=700, height=480, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, location=yes, resizable=no, status=no")
    },
    shareToLinkedin: function (title, url) {
        url = encodeURIComponent(url);
        var _u = "https://www.linkedin.com/shareArticle?mini=true&url=" + url;
        window.open(_u, "", "width=700, height=480, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, location=yes, resizable=no, status=no")
    },
    shareToSina: function (title, url, pic) {
        url = encodeURIComponent(url);
        var _u = 'http://service.weibo.com/share/share.php?title=' + title + '&url=' + url + '&source=bookmark&pic=' + pic;
        window.open(_u, "", "width=700, height=480, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, location=yes, resizable=no, status=no")
    },
    shareToQQ: function (title, url, pic, desc) {
        url = encodeURIComponent(url);
        var _u = 'http://connect.qq.com/widget/shareqq/index.html?url=' + url + '&desc=' + desc + '&summary=' + title + '&site=zeshlife&pics=' + pic;
        window.open(_u, "", "width=700, height=480, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, location=yes, resizable=no, status=no")
    },
    shareToDb: function (title, url, pic, desc) {
        url = encodeURIComponent(url);
        var _u = 'http://www.douban.com/share/service?image=' + pic + '&href=' + url + '&name=' + title + '&text=' + desc;
...
$(document).ready(function () {
    $('.login-required').bind("click", function () {
        if (!login) {
            if (is_mobile)
                window.location.href = "http://lvv2.com/login";
            else {
                $('#login-popup').show();
                $('#capimage').click();
            }
    ...
    $('button').bind("click", function () {
        $(this).siblings('.throbber').addClass('working');
    });
    //$("body").click(close_menus);

    $('.share-twitter-icon').bind("click", function () {
        var t = $(this).closest(".share-icon").attr("title");
        var h = $(this).closest(".share-icon").attr("share-url");
        shareTo.shareToTwitter(t, h);
    });
    $('.share-fb-icon').bind("click", function () {
        var t = $(this).closest(".share-icon").attr("title");
        var h = $(this).closest(".share-icon").attr("share-url");
        shareTo.shareToFacebook(t, h);
    });
    $('.share-linkedin-icon').bind("click", function () {
        var t = $(this).closest(".share-icon").attr("title");
        var h = $(this).closest(".share-icon").attr("share-url");
        shareTo.shareToLinkedin(t, h);
    });

    $('.share-sina-icon').bind("click", function () {
        var t = $(this).closest(".share-icon").attr("title");
        var h = $(this).closest(".share-icon").attr("share-url");
        var img = $('meta[property="og:image"]').attr('content');
        shareTo.shareToSina(t, h, img);
    });
    $('.share-qq-icon').bind("click", function () {
        var t = $(this).closest(".share-icon").attr("title");
        var h = $(this).closest(".share-icon").attr("share-url");
        var desc = $('meta[name="description"]').length ? $('meta[name="description"]').attr('content') : ''

        var img = $('meta[property="og:image"]').attr('content');
        shareTo.shareToQQ(t, h, img, desc);
    });
    $('.share-db-icon').bind("click", function () {
        var t = $(this).closest(".share-icon").attr("title");
        var h = $(this).closest(".share-icon").attr("share-url");
        var desc = $('meta[name="description"]').length ? $('meta[name="description"]').attr('content') : ''
        var img = $('meta[property="og:image"]').attr('content');

        shareTo.shareToDb(t, h, img, desc);
    });
    $('#search_form').bind('submit', function (event) {
        ...
        $inputs.each(function () {
            resultUrl = resultUrl.replace('{' + this.name + '}', $(this).val())
        });
    ...
    $('.navigator-share').click(function (){
        var share = {
            "title": $.trim($(this).parents('.link').find('.nm').text()),
            "url": $(this).data('url')
        }
    ...
    function copyToClipboard() {
        try {
            if (document.execCommand("Copy")) {
                show_message('复制链接成功!', 2, 'rightBox');
            } else {
                show_message('复制失败！请手动复制！', 2, 'alertBox');
            }
        } catch (err) {
            show_message('复制失败！请手动复制！', 2, 'alertBox');
        }
...
if (isAndroid()) {
    if (container = document.getElementById('item-container')) {
        container.className = "twis main lite  isAndroid";
    }
    //document.getElementsByTagName("address")[0].style.fontSize="8px";
}

//////////////////////////////////
//<script type="text/javascript" src="/templates/default/js/dialog/dialog.js" id="dialog_js"></script>
...
__DIALOG_WRAPPER__ = {};
__ScreenLocker_HandleKey__ = true;//null 锁定屏幕 true不锁定屏幕
...
Dialog.prototype = {
    ...
    'createContents': function (type, options) {
        ...
        switch (type) {
            case 'ajax':
                /* 通过Ajax取得HTML，显示到页面上*/
                var _ajax_url = options.url + (options.url.includes('?') ? '&' : '?') + 'handle_key=' + this.id;
                var _ajaxResponse = function (data) {
                    ...
                    if (options.cacheAjaxId && typeof (data) == 'string') {//缓存备用，二次请求直接返回
                        Cache.save(options.cacheAjaxId, data);
                    }
            ...
            case 'message':
                ...
                switch (type) {
                    ...
                    case 'warning':
                        ...
                        _ok_button = $('<input type="button" class="btnText" value="' + button_name + '" />');
                    ...
                    case 'confirm':
                        ...
                        _yes_button = $('<input type="button" class="btnText" value="' + yes_button_name + '" />');
...
////////////////////////////////////////////////////////////
//<script type="text/javascript" src="/templates/default/js/dialog.js.js"></script>
 ...
 * showDialog('loading', 'local', '提示', {'html':'<p class="loading" style="padding:15px 20px 15px 30px; font-size:14px;background-color:#fff;text-indent:30px;">正在切换中，请稍候...</p>'"noTitleBar":true},300);
 *
 */
...
/**
 * 关闭对话框
 * 需要将__DialogHtml__[handle_key] 值空不然弹出窗会有重复内容
 */
function closeDialog(handle_key) {
    if (!isUndefined(__DialogHtml__[handle_key])) {
        __DialogHtml__[handle_key] = undefined;
    }
...
/* rightBox alertBox errorBox */
function show_message(show_message, show_time, msgBox) {
    var show_time = (undefined == show_time ? 1 : show_time);
    var msgBox = (undefined == msgBox ? 'rightBox' : msgBox);

    var smaHTML = '<div class="msg_content ' + msgBox + '"><sub>' + show_message + '</sub></div>';
    showDialog('show_message_' + msgBox, 'local', '提示', {'html': smaHTML}, 300);
    setTimeout(function () {
        closeDialog('show_message_' + msgBox)
    }, (show_time * 1000));
}
 ...
 *    module='loading'    :需要设定options={text:'正在加载'}
 * @param width            对话框宽。
 * cacheAjaxRes 缓存 ajax 返回内容，避免同一url 多次调用
 * show_ajax_tooltip(handle_key, {url:'ajax.php?mod=topic_manage&code=force_out&uid='+uid}, 400);
 * showDialog(handle_key, 'ajax', title, {"url":ajax_url, "post":post}, 620);
 * showDialog(handle_key, 'local', '评论一下', {'html':r}, 500);
 * showDialog('loading', 'local', '提示', {'html':'<p class="loading" style="padding:15px 20px 15px 30px; font-size:14px;background-color:#fff;text-indent:30px;">正在切换中，请稍候...</p>',"noTitleBar":true},300);
 *
 */
//ajax 加载的浮动窗口
var ajax_tooltip_is_loading = true;
var ajax_tooltip_handle_key = 'show_message_rightBox';
var authort;

function show_ajax_tooltip(obj, options, width) {
    if (!width) {
        width = 400;
    }
    var handle_key = ajax_tooltip_handle_key;
    closeDialog(handle_key);
    var position = $(obj).offset();
    var pos = {left: position.left - 10, top: position.top + 25};
    var _cache_id = 'ajax_cache_' + $(obj).attr('data-id');
    options.cacheAjaxId = _cache_id;

    clearTimeout(authort);
    authort = setTimeout(function () {
        var handle = DialogManager.create(handle_key);
        if (Cache.get(_cache_id) != null) {
            var html = Cache.get(_cache_id);
            handle.setContents(html);
        } else {//ajax
            //加上默认的错误处理
            if (!options.checkerror) {
            ...
            handle.setContents('ajax', options);
        }
        handle.setWidth(width);
        handle.show(pos);
        ajax_tooltip_is_loading = false;
    }, 200);
    obj.onmouseout = function () {
        clear_tooltip_choose();
    };
}

function clear_tooltip_choose() {
    clearTimeout(authort);
    if (ajax_tooltip_is_loading) {
        closeDialog(ajax_tooltip_handle_key);
    }
}
...
//微博悬浮显示已赞用户
//<span onclick="topicdig(241355,89,90,'赞');return false;">
//<a class="topicdig_241355 digusers" id="topicdig_241355" href="javascript:;" value="241355" rel="90" title="赞"> (90)</a>
//</span>
function showdiguser() {
    ...
    if ($('.digusers').length) {
        $('.digusers').hover(function() {
                var tid = $(this).attr("value");
                var num = $(this).attr("rel");
                var _cache_id = 'dig_' + tid;
                ...
                if (num > 0) {
					...
					if (Cache.get(_cache_id) != null) {
						...
						var myAjax = $.post("ajax.php?mod=dig&code=user", {
								tid: tid,
								num: num
							},
							function(d) {
...
function dateFormat(dateline){
    ...
    if (t < 172800 && t >= 86400)
        return '昨天';
    if (t < 259200 && t >= 172800)
        return '前天';
    if (t < 86400 * 30 && t >= 259200)
        return Math.floor(t / 86400) + '天前';
    if (t < 86400 * 30 * 12 && t >= 86400 * 30)
        return Math.floor(t / (86400 * 30)) + '个月前';
    return Math.floor(t / (86400 * 30 * 12)) + '年前';
}


/**
    file 简繁体转换js wordexn.js
 */

var Default_isFT = 0;
var StranIt_Delay = 50;

function StranText(b, a, c) {
    if (b == "" || b == null) {
        return ""
    }
    a = a == null ? BodyIsFt : a;
    if (c) {
        b = b.replace((a ? "简" : "繁"), (a ? "繁" : "简"))
    }
    if (a) {
        return Traditionalized(b)
    } else {
        return Simplized(b)
    }
}
function StranBody(e) {
    if (typeof(e) == "object") {
        var d = e.childNodes
    } else {
        var c = StranLink_Obj.innerHTML.toString();
        if (c.indexOf("简") < 0) {
            BodyIsFt = 1;
            StranLink_Obj.innerHTML = StranText(c, 0, 1)
        } else {
            BodyIsFt = 0;
            StranLink_Obj.innerHTML = StranText(c, 1, 1)
        }
    ...
    for (var b = 0; b < d.length; b++) {
        var a = d.item(b);
        if ("||BR|HR|TEXTAREA|".indexOf("|" + a.tagName + "|") > 0 || a == StranLink_Obj) {
            continue
        }
        if (a.title != "" && a.title != null) {
            a.title = StranText(a.title)
        }
        if (a.alt != "" && a.alt != null) {
            a.alt = StranText(a.alt)
        }
        if (a.tagName == "INPUT" && a.value != "" && a.type != "text" && a.type != "hidden") {
            a.value = StranText(a.value)
        }
        if (a.nodeType == 3) {
            a.data = StranText(a.data)
        } else {
            StranBody(a)
        }
    }
...
function JTPYStr() {
    return "态皑蔼碍爱翱袄奥坝罢摆败颁办绊帮绑镑谤剥饱宝报鲍辈贝钡狈备惫绷笔毕毙闭边编贬变辩辫鳖瘪濒滨宾摈饼拨钵铂驳卜补参蚕残惭惨灿苍舱仓沧厕侧册测层诧搀掺蝉馋谗缠铲产阐颤场尝长偿肠厂畅钞车彻尘陈衬撑称惩诚骋痴迟驰耻齿炽冲虫宠畴踌筹绸丑橱厨锄雏础储触处传疮闯创锤纯绰辞词赐聪葱囱从丛凑窜错达带贷担单郸掸胆惮诞弹当挡党荡档捣岛祷导盗灯邓敌涤递缔点垫电淀钓调迭谍叠钉顶锭订东动栋冻斗犊独读赌镀锻断缎兑队对吨顿钝夺鹅额讹恶饿儿尔饵贰发罚阀珐矾钒烦范贩饭访纺飞废费纷坟奋愤粪丰枫锋风疯冯缝讽凤肤辐抚辅赋复负讣妇缚该钙盖干赶秆赣冈刚钢纲岗皋镐搁鸽阁铬个给龚宫巩贡钩沟构购够蛊顾剐关观馆惯贯广规硅归龟闺轨诡柜贵刽辊滚锅国过骇韩汉阂鹤贺横轰鸿红后壶护沪户哗华画划话怀坏欢环还缓换唤痪焕涣黄谎挥辉毁贿秽会烩汇讳诲绘荤浑伙获货祸击机积饥讥鸡绩缉极辑级挤几蓟剂济计记际继纪夹荚颊贾钾价驾歼监坚笺间艰缄茧检碱硷拣捡简俭减荐槛鉴践贱见键舰剑饯渐溅涧浆蒋桨奖讲酱胶浇骄娇搅铰矫侥脚饺缴绞轿较秸阶节茎惊经颈静镜径痉�
�膿濃農瘧諾歐鷗毆嘔漚盤龐國愛賠噴鵬騙飄頻貧蘋憑評潑頗撲鋪樸譜臍齊騎豈啓氣棄訖牽扡釺鉛遷簽謙錢鉗潛淺譴塹槍嗆牆薔強搶鍬橋喬僑翹竅竊欽親輕氫傾頃請慶瓊窮趨區軀驅齲顴權勸卻鵲讓饒擾繞熱韌認紉榮絨軟銳閏潤灑薩鰓賽傘喪騷掃澀殺紗篩曬閃陝贍繕傷賞燒紹賒攝懾設紳審嬸腎滲聲繩勝聖師獅濕詩屍時蝕實識駛勢釋飾視試壽獸樞輸書贖屬術樹豎數帥雙誰稅順說碩爍絲飼聳慫頌訟誦擻蘇訴肅雖綏歲孫損筍縮瑣鎖獺撻擡攤貪癱灘壇譚談歎湯燙濤縧騰謄銻題體屜條貼鐵廳聽烴銅統頭圖塗團頹蛻脫鴕馱駝橢窪襪彎灣頑萬網韋違圍爲濰維葦偉僞緯謂衛溫聞紋穩問甕撾蝸渦窩嗚鎢烏誣無蕪吳塢霧務誤錫犧襲習銑戲細蝦轄峽俠狹廈鍁鮮纖鹹賢銜閑顯險現獻縣餡羨憲線廂鑲鄉詳響項蕭銷曉嘯蠍協挾攜脅諧寫瀉謝鋅釁興洶鏽繡虛噓須許緒續軒懸選癬絢學勳詢尋馴訓訊遜壓鴉鴨啞亞訝閹煙鹽嚴顔閻豔厭硯彥諺驗鴦楊揚瘍陽癢養樣瑤搖堯遙窯謠藥爺頁業葉醫銥頤遺儀彜蟻藝億憶義詣議誼譯異繹蔭陰銀飲櫻嬰鷹應纓瑩螢營熒蠅穎喲擁傭癰踴詠湧優憂郵鈾猶遊誘輿魚漁娛與嶼語籲禦獄譽預馭鴛淵轅園員圓緣遠願約躍鑰嶽粵悅閱雲鄖勻隕運蘊醞暈韻雜災載攢暫贊贓髒鑿棗竈責擇則澤賊贈紮劄軋鍘閘詐齋債氈盞斬輾嶄棧戰綻張漲帳賬脹趙蟄轍鍺這貞針偵診鎮陣掙睜猙幀鄭證織職執紙摯擲幟質鍾終種腫衆謅軸皺晝驟豬諸誅燭矚囑貯鑄築駐專磚轉賺樁莊裝妝壯狀錐贅墜綴諄濁茲資漬蹤綜總縱鄒詛組鑽緻鐘麼為隻兇準啟闆裡靂餘鍊洩"
}
function Traditionalized(e) {
    var d = "",
        b = JTPYStr(),
        c = FTPYStr();
    for (var a = 0; a < e.length; a++) {
        if (e.charCodeAt(a) > 10000 && b.indexOf(e.charAt(a)) != -1) {
            d += c.charAt(b.indexOf(e.charAt(a)))
        } else {
            d += e.charAt(a)
        }
    }
...
function Simplized(e) {
    var d = "",
        b = JTPYStr(),
        c = FTPYStr();
    for (var a = 0; a < e.length; a++) {
        if (e.charCodeAt(a) > 10000 && c.indexOf(e.charAt(a)) != -1) {
            d += b.charAt(c.indexOf(e.charAt(a)))
        } else {
            d += e.charAt(a)
        }
    }
